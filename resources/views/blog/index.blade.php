                    ...
                    <table class="table table-bordered mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><img width="100px" src="{{asset('uploads/blogs/thumb/'.$b->image)}}" alt=""></td>
                                <td>{{$b->heading_text}}</td>
                                <td>{{$b->description}}</td>
                                <td class="white-space-nowrap">
                                    <a href="{{route(currentUser().'.blog.edit',encryptor('encrypt',$b->id))}}">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <a href="javascript:void()" onclick="$('#form{{$b->id}}').submit()">
                                        <i class="bi bi-trash"></i>
                                    </a>
                                    <form id="form{{$b->id}}" action="{{route(currentUser().'.blog.destroy',encryptor('encrypt',$b->id))}}" method="post">
                                        @csrf
                                        @method('delete')
                                    </form>
                                </td>
